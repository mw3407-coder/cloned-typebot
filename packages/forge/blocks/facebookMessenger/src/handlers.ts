...
function saveResponse(
  data: unknown,
  responseMapping:
    | { item?: string | null; variableId?: string | null }[]
    | undefined,
...
export default [
  ...
  createActionHandler(sendGenericTemplate, {
    server: async ({
      ...
      if (!cards || cards.length === 0)
        return logs.add("At least one card is required.");
  ...
  createActionHandler(sendListTemplate, {
    server: async ({
      ...
      if (!items || items.length < 2)
        return logs.add("At least 2 list items are required.");
  ...
  createActionHandler(sendMediaTemplate, {
    server: async ({
      ...
      if (!recipientId) return logs.add("Recipient PSID is missing.");
      if (!url && !attachmentId)
        return logs.add("Either a URL or Attachment ID is required.");
  ...
  createActionHandler(setGreetingText, {
    server: async ({
      ...
      const body = {
        greeting: [{ locale: locale ?? "default", text: greetingText }],
      };
      try {
        const data = await callProfileApi(pageAccessToken, body, logs);
        if (data) saveResponse(data, responseMapping, variables);
